...
# Odczytywanie z plików jest akcją blokującą asynchroniczne działanie bota
# Żeby zapobiec wstrzymaniu heartbeatu prowadzącym do crasha uruchamiamy takie funkcje w executorze

async def get_prefix_value():
    return await asyncio.get_event_loop().run_in_executor(None, prefixValue)


async def get_version_value():
    return await asyncio.get_event_loop().run_in_executor(None, versionValue)
...
async def le(ctx, l):
    if l is None:
        ...
        plan = await asyncio.get_event_loop().run_in_executor(None, readPlan, day)
...
async def sprawdz(ctx):
    ...
    for person in room:
        ...
        if len(man.roles) > 1:
            ...
            for role in roles:
                if role.id == await asyncio.get_event_loop().run_in_executor(None, readUserTag):
                    i += 1
                    try:
    ...
    for letter in numery:
        numbers = await asyncio.get_event_loop().run_in_executor(None, getNumbers, str(letter))
        if z > 0:
            text += f', {letter}'
            imiona += f', {numbers}'
        else:
